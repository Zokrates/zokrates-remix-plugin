...
import { WA_COMPILE, WA_SETUP, WA_COMPUTE, WA_EXPORT_VERIFIER, WA_GENERATE_PROOF, WA_ERROR } from './zokrates/constants';

initialize()
    .then((zokratesProvider: ZoKratesProvider) => {
        const onCompile = (source: string, location: string, imports: object) => {
            const importResolver = (location: string, path: string): ResolverResult => {
            ...
            try {
                switch (type) {
                    case WA_COMPILE: {
                        ...
                        // @ts-ignore 
                        self.postMessage({ type: type, payload: [artifacts, payload.source] });
                        break;
                    }
                    case WA_COMPUTE: {
